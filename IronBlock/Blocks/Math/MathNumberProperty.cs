...
namespace IronBlock.Blocks.Math
{
    public class MathNumberProperty : IBlock
    {
        public override object Evaluate(Context context)
        {
            ...
            switch (op)
            {
                ...
                case "NEGATIVE":
                    return number < 0;
                case "DIVISIBLE_BY":
                    return 0 == number % (double) Values.Evaluate("DIVISOR", context);
                default:
                    throw new ApplicationException($"Unknown PROPERTY {op}");
            }
        }
        ...
        private SyntaxNode CompareModulo(ExpressionSyntax numberExpression, ExpressionSyntax moduloValueExpression,
            double compareValue)
        {
            return
                BinaryExpression(
